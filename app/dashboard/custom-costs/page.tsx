...
interface CostHead { id: string; name: string; }
...
export default function CustomValuesPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [resUsers, resData] = await Promise.all([
        ...
        api.get(process.env.NEXT_PUBLIC_GAS_URL!, { params: { action: "getCustomValuesData", token } }),
      ]);
  ...
  const handleInputChange = (idx: number, field: keyof CustomValueItem, value: any) => {
    const updated = [...items];
  ...
  const handleSave = async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      if (!items[i].userId || !items[i].costHeadId || items[i].amount === null) {
    ...
    try {
      ...
      await api.post(`${process.env.NEXT_PUBLIC_GAS_URL}?action=upsertCustomValues&token=${token}`, params);
  ...
  return (
    <div className="p-4 md:p-6 bg-white dark:bg-gray-900 min-h-screen text-gray-900 dark:text-gray-100">
      ...
      <div className="flex justify-between items-center mb-6 border-b-2 border-gray-200 dark:border-gray-700 pb-2">
        <h1 className="text-2xl font-bold uppercase tracking-wider">Custom Assignments</h1>
        <div className="flex gap-3">
          {!isEditing ? (
            ...
            <>
              <button onClick={() => setItems([...items, { userId: "", costHeadId: "", amount: null }])} className="px-4 py-2 bg-green-100 dark:bg-green-900/30 border-2 border-green-600 text-green-700 dark:text-green-400 font-black uppercase text-xs">
                + Add Rule
              </button>
              ...
              <button onClick={() => { setIsEditing(false); setItems(JSON.parse(JSON.stringify(originalItems))); }} className="px-4 py-2 border-2 border-gray-300 dark:border-gray-600 font-black uppercase text-xs">
                Cancel
              </button>
            </>
          )}
        ...
        <table className="min-w-full text-sm text-left border-separate border-spacing-0">
          <thead className="bg-gray-200 dark:bg-gray-700 font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">
            <tr>
              ...
              <th className="px-6 py-4 border-b border-gray-300 dark:border-gray-700">Member Name</th>
              <th className="px-6 py-4 border-b border-gray-300 dark:border-gray-700">Cost Head</th>
              <th className="px-6 py-4 border-b border-gray-300 dark:border-gray-700">Fixed Amount</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-300 dark:divide-gray-700">
            {items.map((item, idx) => {
               const isChanged = isEditing && JSON.stringify(item) !== JSON.stringify(originalItems[idx]);
               return (
                <tr key={idx} className={`${idx % 2 === 0 ? "bg-white dark:bg-gray-900" : "bg-gray-100 dark:bg-gray-800/40"} ${isChanged ? "bg-yellow-50 dark:bg-yellow-900/10" : ""}`}>
                  ...
                  <td className="px-6 py-4 border-r border-gray-200 dark:border-gray-800">
                    {isEditing ? (
                      <select className="w-full bg-white dark:bg-gray-800 border dark:border-gray-600 px-2 py-1 rounded outline-none" value={item.userId} onChange={(e) => handleInputChange(idx, "userId", e.target.value)}>
                  ...
                  <td className="px-6 py-4 border-r border-gray-200 dark:border-gray-800">
                    {isEditing ? (
                      <select className="w-full bg-white dark:bg-gray-800 border dark:border-gray-600 px-2 py-1 rounded outline-none" value={item.costHeadId} onChange={(e) => handleInputChange(idx, "costHeadId", e.target.value)}>
                        <option value="">Select Head</option>
                        {costHeads.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                      </select>
                    ) : <span className="font-bold">{costHeads.find(c => String(c.id) === String(item.costHeadId))?.name || "Unknown"}</span>}
                  </td>
                  <td className="px-6 py-4 font-mono">
                    {isEditing ? (
                      <input type="number" className="w-full bg-white dark:bg-gray-800 border dark:border-gray-600 px-2 py-1 rounded outline-none" value={item.amount ?? ""} onChange={(e) => handleInputChange(idx, "amount", e.target.value === "" ? null : Number(e.target.value))} placeholder="0.00" />
                    ) : <span className="font-bold text-orange-600 dark:text-orange-400 italic">{item.amount?.toLocaleString()}</span>}
                  </td>
                </tr>
              );
            })}
